...
<div id="body-pd">
    ...
    <header class="header" id="header">
        ...
        <h4>Your Added Voice</h4>
        <a id="arch" class="text-white btn btn-info btn-sm"><i class="fas fa-trash"></i> My archived audio</a>
    </header>
          ...
          <div style="overflow:auto;" class="table-scrollable">
            <table id="example1" width="100%" class="table table-bordered table-striped">
              ...
              <tbody>
                ...
                $sql = mysqli_query(
                  $conn,
                  "select * from button where status = 1 AND user = '$user' order by id desc"
                );
                $i = 1;
                while ($result = mysqli_fetch_array($sql)) {
                  ...
                  <tr>
                    ...
                    <td>
                      ...
                      $badgeClass = ($status == 2) ? 'badge-warning' : 'badge-success';
                      $statusText = ($status == 2) ? 'Unpublish' : 'Publish';
                      ?>
                    ...
                    <td>
                      ...
                      <a voicearchive="<?php echo $result['id']; ?>" class="text-white btn btn-danger btnarchive btn-sm"
...
Swal.fire({
    title: 'Archived Page',
  text: 'Are you sure you want to go Archived page?',
  icon: 'warning',
  showCancelButton: true,
  ...
  if (result.isConfirmed) {
    // Perform logout actions here
    window.location.href = "./userarchived.php"; // Redirect to the logout page
  }
});
});
...
<script>
  ...
  var voicearchive = $(this).attr('voicearchive');
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      archiveEvent(voicearchive, password);
    } else {
...
function archiveEvent(voicearchive, password) {
  if (password === '') {
